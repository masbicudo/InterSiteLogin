...
namespace Masb.Yai
{
    ...
    internal class TypeReplacementVisitor : ExpressionVisitor
    {
        ...
        private ConstructorInfo VisitConstructorInfo(ConstructorInfo ctor)
        {
            ...
            {
                Debugger.Break();
                var result = type2.GetConstructors().Single(c => c.MethodHandle == ctor.MethodHandle);
                return result;
            }
        ...
        private MethodInfo VisitMethodInfo(MethodInfo mi)
        {
            ...
            {
                Debugger.Break();
                newMi = type2.GetMethods().Single(c => c.MethodHandle == mi.MethodHandle);
            }
        ...
        protected override Expression VisitNew(NewExpression node)
        {
            ...
            return newConstructor != node.Constructor || !args.SequenceEqual(node.Arguments)
                ? Expression.New(newConstructor, args)
                : node;
        }
